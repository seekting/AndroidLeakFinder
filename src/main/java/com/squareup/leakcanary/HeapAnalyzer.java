...
public final class HeapAnalyzer {
    ...
    public @NonNull
    AnalysisResult checkForLeak1(@NonNull File heapDumpFile,
        ...
        try {
            ...
            deduplicateGcRoots(snapshot);
            listener.onProgressUpdate(FINDING_LEAKING_REF, "");
            List<ClassInstance> leakingRefs = findReference(clazzName, snapshot);

            // False alarm, weak reference was cleared in between key check and heap dump.
            if (leakingRefs == null || leakingRefs.size() == 0) {
                return noLeak(since(analysisStartNanoTime));
            }
            return findLeakTrace(analysisStartNanoTime, snapshot, leakingRefs.get(0), computeRetainedSize);
        } catch (Throwable e) {
            return failure(e, since(analysisStartNanoTime));
        }
    }
    ...
    public @NonNull
    List<AnalysisResult> checkForLeaks(@NonNull File heapDumpFile,
                                       @NonNull String clazzName,
                                       boolean computeRetainedSize) {
        ...
        if (!heapDumpFile.exists()) {
            Exception exception = new IllegalArgumentException("File does not exist: " + heapDumpFile);
            results.add(failure(exception, since(analysisStartNanoTime)));
            return results;
        ...
        try {
            ...
            deduplicateGcRoots(snapshot);
            listener.onProgressUpdate(FINDING_LEAKING_REF, "");
            List<ClassInstance> leakingRefs = findReferenceSubs(clazzName, snapshot);

            // False alarm, weak reference was cleared in between key check and heap dump.
            if (leakingRefs == null || leakingRefs.size() == 0) {
                results.add(noLeak(since(analysisStartNanoTime)));
            }
            for (ClassInstance leakingRef: leakingRefs) {
                AnalysisResult j = findLeakTrace(analysisStartNanoTime, snapshot, leakingRef, computeRetainedSize);
                results.add(j);
    ...
    private List<ClassInstance> findReferenceSubs(String clazzName, Snapshot snapshot) {
        ClassObj refClass = snapshot.findClass(clazzName);
        if (refClass == null) {
            throw new IllegalStateException(
                    "Could not find the " + refClass + " class in the heap dump.");
        }
        List<ClassObj> list = new ArrayList<>();
        Stack<ClassObj> stack = new Stack<>();
        stack.push(refClass);
        while (!stack.isEmpty()) {
            ClassObj classObj = stack.pop();
            list.add(classObj);
            Set<ClassObj> set = classObj.getSubclasses();

            if (set != null) {
                for (ClassObj obj: set) {
                    stack.push(obj);
                }
            }
        ...
        List<ClassInstance> mDestoryedActivitys = new ArrayList<>();
        List<String> keysFound = new ArrayList<>();
        for (ClassObj r: list) {
            for (Instance instance: r.getInstancesList()) {
                ClassInstance classInstance = (ClassInstance) instance;
                List<ClassInstance.FieldValue> fieldValues = classInstance.getValues();
                boolean mDestroyed = fieldValue(fieldValues, "mDestroyed");
                if (mDestroyed) {
                    if (!mDestoryedActivitys.contains(classInstance)) {
                        mDestoryedActivitys.add(classInstance);
                    }
        ...
        return mDestoryedActivitys;
    }

    private List<ClassInstance> findReference(String clazzName, Snapshot snapshot) {
        ClassObj refClass = snapshot.findClass(clazzName);
        if (refClass == null) {
            throw new IllegalStateException(
                    "Could not find the " + refClass + " class in the heap dump.");
    ...
    public AnalysisResult findLeakTrace(long analysisStartNanoTime, Snapshot snapshot,
        ...
        listener.onProgressUpdate(FINDING_SHORTEST_PATH, leakingRef.toString());
